...
public class ProviderDaoImpl implements ProviderDao {
	...
	public int add(Connection connection, Provider provider)
		...
		if(null != connection){
			String sql = "insert into smbms_provider (proCode,proName,proDesc," +
					"proContact,proPhone,proAddress,proFax,createdBy,creationDate,idPicPath,workPicPath) " +
					"values(?,?,?,?,?,?,?,?,?,?,?)";
			Object[] params = {provider.getProCode(),provider.getProName(),provider.getProDesc(),
								...
								provider.getProFax(),provider.getCreatedBy(),provider.getCreationDate(),provider.getIdPicPath(),provider.getWorkPicPath()};
	...
	@Override
	public int queryProviderByCode(Connection connection, String proCode) throws Exception {
		PreparedStatement pstm = null;
		int res =-1;
		if(null != connection){
