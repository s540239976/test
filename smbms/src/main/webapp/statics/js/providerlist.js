...
$(function(){
		...
		$.ajax({
			...
			success:function (data) {
				if(data == "null"){
					...
					$(".providerView #contact").html(data.proContact);
					$(".providerView #phone").html(data.proPhone);
					$(".providerView #profix").html(data.proFax);
	...
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/modifyProvider/"+ obj.attr("proid");
	});
