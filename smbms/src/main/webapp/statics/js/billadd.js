...
$(function() {
	...
	billCode.bind("blur",function () {
		$.ajax({
			type:"GET",
			url:path+"/bill/hasBillCode",
			data:{billCode:billCode.val()},
			dataType:"json",
			success:function (data) {
				if(data.billCode == "exist"){
					validateTip(billCode.next(),{"color":"red"},imgNo+ " the billCode has existed",false);
				}else{
					validateTip(billCode.next(),{"color":"green"},	imgYes+" ok",true);
				}
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/bill/queryProvider",//请求的url
