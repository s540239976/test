...
$(function(){
		...
		$.ajax({
			...
			success:function (data) {
				if(data == "null"){
					...
					$(".providerView #proName").html(data.productName);
					$(".providerView #proUnit").html(data.productUnit);
					$(".providerView #proCount").html(data.productCount);
					$(".providerView #total").html(data.totalPrice);
					$(".providerView #provider").html(data.providerName);
					$(".providerView #isPay").html(data.isPayment);
				}
			}
		})
	});
	...
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/bill/modifyBill?billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
